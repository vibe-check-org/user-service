...
export class KeycloakAdminService {
    ...
    async updatePassword(userId: string, newPassword: string, adminToken: string) {
        const passwordData = {
            type: 'password',
            value: newPassword,
            temporary: false,
        ...
        try {
            await this.#client.put(`${paths.admin}/${userId}/reset-password`, passwordData, {
                headers: {
                    Authorization: `Bearer ${adminToken}`,
                    'Content-Type': 'application/json',
                },
    ...
    async assignRoleToUser(userId: string, roleName: string, adminToken: string) {
        ...
        try {
            await this.#client.post(`${paths.admin}/${userId}/role-mappings/realm`, roleData, {
                headers: {
                    Authorization: `Bearer ${adminToken}`,
                    'Content-Type': 'application/json',
                },
    ...
    async getUserIdByUsername(token: string, username: string): Promise<string> {
        ...
        if (!user) throw new NotFoundException(`Benutzer nicht gefunden: ${username}`);
        return user.id;
    }
